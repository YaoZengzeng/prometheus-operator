...
// Prometheus定义了一个Prometheus deployment
// +genclient
...
// https://github.com/kubernetes/community/blob/master/contributors/devel/api-conventions.md#spec-and-status
// +k8s:openapi-gen=true
// PrometheusSpec是关于Prometheus cluster的期望行为的说明
type PrometheusSpec struct {
	...
	// ServiceMonitors被选择用于target discovery
	ServiceMonitorSelector *metav1.LabelSelector `json:"serviceMonitorSelector,omitempty"`
	// Namespaces to be selected for ServiceMonitor discovery. If nil, only
	// check own namespace.
	// 用于ServiceMonitor发现的Namespace，如果为nil，则只检查自己的namespace
	...
	// 一个selector用于选择挂载哪个PrometheusRuls来加载alerting rules
	RuleSelector *metav1.LabelSelector `json:"ruleSelector,omitempty"`
	// Namespaces to be selected for PrometheusRules discovery. If unspecified, only
	// the same namespace as the Prometheus object is in is used.
	// 用于PrometheusRules discovery的Namespace，如果没有指定，就只会使用Prometheus对象所在的namespace
	...
	// object, which shall be mounted into the Prometheus Pods.
	// ConfigMaps是一系列和Prometheus object在同一个namespace的ConfigMaps，它们应该被挂载到Prometheus Pods中
	// The ConfigMaps are mounted into /etc/prometheus/configmaps/<configmap-name>.
	// ConfigMaps会被挂载到/etc/prometheus/configmaps/<configmap-name>
	...
	// ListenLocal让Prometheus server监听在loopback，因此它就不会bind到Pod IP
	...
	// AdditionalScrapeConfigs允许指定一个Secret的key，包含额外的Prometheus scrap configurations
	// 指定的Scrape configurations会扩展到由Prometheus Operator产生的配置
	...
	// APIServerConfig允许指定一个host以及auth methods用于访问apiserver
	// 如果它为空，Prometheus假设运行在cluster中并且自动发现API Servers并且使用pod的CA certificate
	// bearer toke file在/var/run/secrets/kuernetes.io/serviceaccount
...
// StorageSpec为一系列的Prometheus servers定义configured storage
// If neither `emptyDir` nor `volumeClaimTemplate` is specified, then by default an [EmptyDir](https://kubernetes.io/docs/concepts/storage/volumes/#emptydir) will be used.
// 如果`emptyDir`和`volumeClaimTemplate`都没有定义，则默认使用EmptyDir
// +k8s:openapi-gen=true
type StorageSpec struct {
	// EmptyDirVolumeSource to be used by the Prometheus StatefulSets. If specified, used in place of any volumeClaimTemplate. More
	// info: https://kubernetes.io/docs/concepts/storage/volumes/#emptydir
	// EmptyDirVolumeSource由Prometheus StatefulSets使用，如果指定了，替代任何的volumeClaimTemplate
...
// RemoteWriteSpec定义了prometheus的remote_write配置
// +k8s:openapi-gen=true
...
// ServiceMonitorSpec包含了一个ServiceMonitor的specification parameters
// +k8s:openapi-gen=true
type ServiceMonitorSpec struct {
	// The label to use to retrieve the job name from.
	JobLabel string `json:"jobLabel,omitempty"`
	// TargetLabels transfers labels on the Kubernetes Service onto the target.
	// TargetLabels转换Kubernetes Service的labels转换为target
	TargetLabels []string `json:"targetLabels,omitempty"`
	// PodTargetLabels transfers labels on the Kubernetes Pod onto the target.
	// PodTargetLabels转换Kubernetes Pod的labels转换为target
...
// Endpoint定义了一个scrapeable endpoint，用于提供Prometheus metrics
// +k8s:openapi-gen=true
...
type AlertmanagerSpec struct {
	...
	// configured.
	// 如果指定了Image，那么它的优先级比baseImage+tag+sha组合的优先级高
	Image *string `json:"image,omitempty"`
	...
	// Secrets是一系列和Alertmanager在同一个namespace的Secrets
	// 它们都应该被挂载在Alertmanager Pods上
	...
	// Alertmanager应该保留数据的时间，默认为'120h'
	// and must match the regular expression `[0-9]+(ms|s|m|h)` (milliseconds seconds minutes hours).
	Retention string `json:"retention,omitempty"`
	// Storage is the definition of how storage will be used by the Alertmanager
	// instances.
	// Storage定义了Alertmanager实例如何使用存储
	Storage *StorageSpec `json:"storage,omitempty"`
	...
	// AdditionalPeers允许额外注入一系列的Alertmanagers，用于构成一个高可用的clusters
