...
func (c *Operator) createOrUpdateRuleConfigMapsNew(sset *appsv1.StatefulSet, rules []*monitoringv1.PrometheusRule) ([]string, error) {
	...
	level.Debug(c.logger).Log(
		...
		"statefulset", sset.Name,
	)

	currentConfigMapList, err := cClient.List(prometheusRulesConfigMapSelector(sset.Name))
	if err != nil {
		return nil, err
	}
	...
	newConfigMaps, err := makeRulesConfigMapsNew(sset, newRules)
	if err != nil {
...
func makeRulesConfigMapsNew(sset *appsv1.StatefulSet, ruleFiles map[string]string) ([]v1.ConfigMap, error) {
	//check if none of the rule files is too large for a single ConfigMap
	for filename, file := range ruleFiles {
		if len(file) > maxConfigMapDataSize {
			return nil, errors.Errorf(
	...
	ruleFileConfigMaps := []v1.ConfigMap{}
	for i, bucket := range buckets {
		cm := makeRulesConfigMapNew(sset, bucket)
		cm.Name = cm.Name + "-" + strconv.Itoa(i)
		ruleFileConfigMaps = append(ruleFileConfigMaps, cm)
	}
...
func makeRulesConfigMapNew(sset *appsv1.StatefulSet, ruleFiles map[string]string) v1.ConfigMap {
	boolTrue := true

	labels := map[string]string{labelPrometheusName: sset.Name}
	for k, v := range managedByOperatorLabels {
		labels[k] = v
	}
	...
	return v1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:   prometheusRuleConfigMapName(sset.Name),
			Labels: labels,
			OwnerReferences: []metav1.OwnerReference{
				{
					APIVersion:         sset.APIVersion,
					BlockOwnerDeletion: &boolTrue,
					Controller:         &boolTrue,
					Kind:               sset.Kind,
					Name:               sset.Name,
